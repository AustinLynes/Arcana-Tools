...
namespace ArcanaTools {
	...
	void Model::loadMesh(unsigned int indMesh)
	{
		...
		std::vector<Vertex3D> vertices = AssembleVertices(positions, normals, texUVs);
		std::vector<GLuint> indices = getIndices(JSON["accessors"][indAccInd]);
		std::vector<Texture2D*> textures = GetTextures();
	...
	std::vector<Texture2D*> Model::GetTextures()
	{
		...
		for (unsigned int i = 0; i < JSON["images"].size(); i++)
		{
			...
			if (!skip)
			{
				...
				{
					Texture2D* diffuse = Texture2D::Create((fileDirectory + texPath).c_str(), "diffuse", loadedTex.size());
				...
				{
					Texture2D* specular = Texture2D::Create((fileDirectory + texPath).c_str(), "specular", loadedTex.size());
